...
record is its Status line, which is updated to point at the record that
superseded it, so that a reader arriving at the old record learns immediately
...
Records, and Design Documents alike, including titles, slugs, and section
bodies.
...
existing information, so the cost of a bad entry is low and correctable.
...
Status: accepted | superseded by DR-____
Date: YYYY-MM-DD
