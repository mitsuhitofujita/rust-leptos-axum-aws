...
| `crates/app/src/action_types.rs` | `/action-types` and `/action-types/new` — the index, and registering one |
...
(`NewActionTypePage`) and `/action-types` (`ActionTypesPage`), with a `NotFound`
fallback. Navigation uses `<A>`, which renders `aria-current="page"` on the
active link — the CSS styles that attribute rather than tracking the active
route by hand.

Two paths reachable by design still land on the fallback, because their screens
do not exist: editing an action type, which an index row links to, and recording
an action, which a dashboard row links to.
...
Writes do not go through a resource. `create_action_type` is called from an
event handler in `spawn_local`, with the screen holding its own `saving` and
`error` signals, because a submission is something the visitor did once rather
than something the screen is a view of.
...
**Consumes** `GET /api/dashboard`, `GET /api/action-types` and
`POST /api/action-types` — see [Backend](backend.md) — as absolute paths joined
